...
export class GetStats {
    async execute(dto) {
        ...
        const completedWorkoutsCount = allSessions.filter((s) => s.completedAt !== null).length;
        // conclusionRate
        const conclusionRate = allSessions.length > 0 ? completedWorkoutsCount / allSessions.length : 0;
        // totalTimeInSeconds
        ...
        // workoutStreak
        const workoutStreak = this.calculateStreak(workoutPlan.workoutDays, dayjs.utc(dto.to));
        return {
            workoutStreak,
            consistencyByDay,
            completedWorkoutsCount,
    ...
    calculateStreak(workoutDays, currentDate) {
        let streak = 0;
        ...
        while (true) {
            ...
            const hasCompletedSession = workoutDay.sessions.some((s) => {
                if (!s.completedAt)
                    return false;
                return (dayjs.utc(s.completedAt).format("YYYY-MM-DD") ===
